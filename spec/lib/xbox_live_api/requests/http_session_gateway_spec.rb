...
describe XboxLiveApi::Requests::HttpSessionGateway do
  let(:client) { double }
  ...
  describe '#get' do
    ...
    it 'returns a HttpResponse with string body' do
      response = XboxLiveApi::Requests::HttpResponse.new(response_body, header, status)
      expect(subject.get(uri)).to eq response
    end
  end
  ...
  describe '#post' do
    ...
    it 'returns a HttpResponse with string body' do
      response = XboxLiveApi::Requests::HttpResponse.new(response_body, header, status)
      expect(subject.post(uri)).to eq response
    end
  end
  ...
  describe '#post_json' do
    ...
    it 'returns a HttpResponse with string body' do
      response = XboxLiveApi::Requests::HttpResponse.new(response_body, header, status)
      expect(subject.post_json(uri)).to eq response
    end
  end
